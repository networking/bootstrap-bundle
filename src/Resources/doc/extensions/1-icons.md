...
```jinja
Regular Icon - {{ networking_bootstrap_icon('pencil') }}
Inversed Icon - {{ networking_bootstrap_icon('pencil', true) }}
```
...
Please note that you must not use the `@NetworkingBootstrap/Resources/public/less/mopabootstrapbundle.less` annotation or the css rewrite will fail.
